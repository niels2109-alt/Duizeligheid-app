# Technische Requirements — BPPV-MVP
...
**Besluit**: automatisch verwijderen 90 dagen na sessie-einde, of direct zodra de therapeut heeft geëxporteerd (dan al overgenomen in eigen EPD, kopie hier niet langer nodig). **Waarom**: 90 dagen dekt een realistisch BPPV-vervolgtraject (doorgaans 1-3 sessies, enkele weken) ruim, zonder tegen AVG-dataminimalisatie in te gaan. Geen handmatige actie van de therapeut vereist — verval gebeurt automatisch.
...
**Stap 1 — Datamodel + BPPV-content**
Database opzetten volgens §1, gevuld met de volledige BPPV-content uit `Duizeligheid-Referentiemodel.md` (§2-6): AAND-001, alle subtypes, TEST-001/002, INT-001 t/m 004, RF-001 t/m 006, differentiaaldiagnose-relaties, EDU-001, PEARL-001. Oplevering: een gevulde database, nog geen interface.

**Stap 2 — Modus B: kennisbank raadplegen**
Eenvoudigste functionaliteit, geen flow-logica — goede eerste test of stap 1 correct werkt. Zoek-/filterinterface over de gevulde BPPV-data (§2.2).

**Stap 3 — Modus A: reasoning-flow**
De kern van het product, de zeven stappen uit §2.1, bovenop de in stap 1-2 geteste data. Voor BPPV is `uitkomsttype = enkelvoudig`, dus de samengesteld-uitkomst-logica (relevant voor het latere ouderenduizeligheid-item) hoeft in deze eerste bouwronde nog niet gebouwd te worden.
